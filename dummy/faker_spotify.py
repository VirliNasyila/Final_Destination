...
def generate_data_safe():
    ...
    for _ in range(15):
        ...
        # Generate password random (10-15 karakter, alphanum)
        raw_pw = fake.password(length=12, special_chars=True, digits=True, upper_case=True, lower_case=True)
        
        # Escape single quote manual karena masuk ke dalam string SQL function
        safe_raw_pw = raw_pw.replace("'", "''")
        
        # Buat expression SQL nya
        pw_expression = f"crypt('{safe_raw_pw}', gen_salt('bf'))"

        sql = f"INSERT INTO USERS (USER_ID, USERNAME, USER_EMAIL, PW_HASH, USER_PFP, REGION, COUNTRY) VALUES ({ids['users']}, {escape_sql(u_name)}, {escape_sql(email)}, {pw_expression}, {escape_sql(pfp)}, {escape_sql(region)}, {escape_sql(country)});"
        
        # Fitur Tambahan: Tulis password asli di comment SQL biar bisa buat login test
        sql += f" -- Password Asli: {raw_pw}"
    ...
    for u_id in valid_users:
        for _ in range(random.randint(1, 3)):
            ...
            pl_cover = f"https://via.placeholder.com/150?text={pl_title.split()[0]}"

            sql_pl = f"INSERT INTO PLAYLISTS (PLAYLIST_ID, USER_ID, PLAYLIST_TITLE, ISPUBLIC, ISCOLLABORATIVE, PLAYLIST_DESC, ISONPROFILE, PLAYLIST_DATE_CREATED, PLAYLIST_COVER) VALUES ({playlist_id_counter}, {u_id}, {escape_sql(pl_title)}, {is_public}, {is_collab}, {escape_sql(desc)}, {on_profile}, '{date_created}', {escape_sql(pl_cover)});"
